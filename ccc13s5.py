...
In the game of Factor Solitaire, you start with the number 1, and try to change it to some given target number n by repeatedly using the following
operation. In each step, if c is your current number, you split it into two positive factors a, b of your choice such that c = a x b. You then add a to
your current number c to get your new current number. Doing this costs you b points.
...
def get_factors(n):
    if n in factors_pool:
        return factors_pool[n]
    factors_pool[n] = list(factor for i in range(1, int (n**0.5) + 1) if n % i == 0 for factor in (i, n//i)) 
    return factors_pool[n]

# cache all search results of one number
# key is number, value is the min cost path of key
# a XXXX number have tens factors. if a length of path is 10, the try times maybe too huge.
# So cache every lowest cost path of any number to target_num, can save many searching.
search_lowcost_pool = {}

# from current integer, try every factor of current integer until equel target integer
...
def search_lowcost_path(from_num, to_num):
    ...
    for factor in factors:
        ...
        # try a sub path
            ...
            if (sub_path != None and len(sub_path) > 0):
                sub_path_score = sum(list(zip(*sub_path))[1])   # can cache too from optimized
    ...
    # if the lowest cost path which from from_num to to_num, cache the lowest cost path of [from_num]
